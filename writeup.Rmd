...
adi <- fread("adi-first.csv")[,date := as.Date(date)]
# Trim these to the same dates
# First ADI
ms.adi <- ms.ww[date >= min(adi$date) & date <= max(adi$date)]
df.adi <- data.table(date = ms.adi$date, usage = adi$adi, share = ms.adi$Firefox)
...
res <- rbind(res.ww, res.us, res.adi)
```

```{r eval=TRUE,fig.width=6, fig.height=4, echo=FALSE}
ggplot(res) + geom_line(mapping=aes(x=date,y=size.normal, color=series))
```
